...
function App() {
  ...
  useEffect(() => {
    ...
    const x = setInterval(() => {
      setCookies(prev => [...prev, { speed: Math.random() * (2 - 1) + 1, xValue: Math.random() * (89 - 3) + 3 }])
    }, 2000)
    return () => {
  ...
  function deleteCookie(data) {
    setCookies(prev => prev.filter(c => (c.xValue !== data.xValue && c.speed !== data.speed)))
  }
  return (
    <div className="main-container">
      <div className="inner-container">
        ...
        {
          cookies.map(cookie => {
            return <Cookie speed={cookie.speed} xValue={cookie.xValue} deleteCookie={deleteCookie} setScore={setScore} basketRef={basketRef} />
          })
        }
        <div className="basket" ref={basketRef}></div>
      </div>
      <div className="button-container">
        <div className="button" onTouchStart={() => handleKeyDownEvent({ key: 'ArrowLeft' })} onTouchEnd={() => handleKeyUpEvent({ key: 'ArrowLeft' })}>
          <img src={leftButton} alt='LEFT' />
        </div>
        <div className="button" onTouchStart={() => handleKeyDownEvent({ key: 'ArrowRight' })} onTouchEnd={() => handleKeyUpEvent({ key: 'ArrowRight' })}>
